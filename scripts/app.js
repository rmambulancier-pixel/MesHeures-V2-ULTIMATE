import {calcDay as V2calcDay, calcPeriod as V2calcPeriod} from './core/engine.js';
...
function cd(k){ return V2calcDay(k,DB.days[k],DB.s,isFerie); }
function calcPer(start,nb){ return V2calcPeriod(start,nb,DB.days,DB.s,isFerie); }
function brutOf(G){
...
function tab(t){
  ...
  ['home','jour','paie','audit','more'].forEach(x=>$('b-'+x)?.classList.toggle('on',(x==='more'?t==='reg':x===t)));
  window.scrollTo(0,0);
  renderAll();
}
...
function renderHome(){
  const todayKey=today(), d=cd(todayKey);
  const dayLabel=new Intl.DateTimeFormat('fr-FR',{weekday:'long',day:'numeric',month:'long',year:'numeric'}).format(new Date());
  $('homeDate').textContent=dayLabel.charAt(0).toUpperCase()+dayLabel.slice(1);
  $('homeToday').textContent=d.t==='T'?F(d.tte):d.t||'Repos';
  const {AL,G}=calcPer(DB.per.start,DB.per.nb);
  $('homePeriod').textContent=F(G.tte);
  const critical=AL.filter(a=>a.lvl==='b').length;
  const warnings=AL.filter(a=>a.lvl==='w').length;
  $('homeAlerts').textContent=critical?critical+' critique(s)':warnings?warnings+' à vérifier':'RAS';
  $('homeProofs').textContent=(DB.bulletins?.length||0)+(Object.keys(DB.romi||{}).length||0)+' élément(s)';
  $('homeHealth').textContent=critical?'⚠️ À CONTRÔLER':warnings?'🔎 À VÉRIFIER':'🟢 DONNÉES OK';
  const B=brutOf(G);
  $('homeSummary').innerHTML=
    '<div class="summary-box"><small>TTE période</small><strong>'+F(G.tte)+'</strong></div>'+
    '<div class="summary-box"><small>Amplitude période</small><strong>'+F(G.amp)+'</strong></div>'+
    '<div class="summary-box"><small>Brut estimé</small><strong>'+EUR(B.tot)+'</strong></div>'+
    '<div class="summary-box"><small>Heures supplémentaires</small><strong>'+F(G.h25+G.h50)+'</strong></div>'+
    '<div class="summary-box"><small>RC</small><strong>'+F(G.rc)+'</strong></div>'+
    '<div class="summary-box"><small>Jours travaillés</small><strong>'+G.trav+'</strong></div>';
}
